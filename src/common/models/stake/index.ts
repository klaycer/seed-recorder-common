...
import { DBNAME, STAKE_RECORD_LIMBO_BET,
  STAKE_RECORD_CRASH_BET, STAKE_RECORD_DICE_BET,
...
export class StakeDB extends Dexie {
  ...
  constructor() {
    ...
    const defaultModelDefinition = "++id, active, amount, currency, amountMultiplier, game, payout, payoutMultiplier, state, updatedAt, user, createdAt";
  ...
  addResponseByModel = async (model: Table<StakeDefault, number>, result: StakeDefault) => {
  ...
  addCrashResult = async (result: StakeCrashResult) => {
    if(result.status === "ended"){
      return await this.stakeCrashResult.add({
        id: result.id,
  ...
  addCrashBet = async (result: StakeCrashBet) => {
    return await this.stakeCrashBet.add({
      id: result.id,
